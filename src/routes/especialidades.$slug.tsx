  ...
  return (
    <SiteShell>
      <main>
        ...
        <section className="mx-auto grid max-w-6xl gap-10 px-5 py-16 sm:px-6 md:py-24 lg:grid-cols-[220px_minmax(0,720px)] lg:justify-center">
          <aside className="lg:sticky lg:top-28 lg:self-start"><p className="eyebrow">Nesta página</p><nav className="mt-4 flex flex-wrap gap-x-5 gap-y-3 border-t border-border pt-4 text-sm text-muted-foreground lg:flex-col">{specialty.articles.map((article) => <a key={article.id} href={`#${article.id}`} className="hover:text-secondary">{article.title}</a>)}</nav></aside>
          <div>
